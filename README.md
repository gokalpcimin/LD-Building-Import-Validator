...
| Import-ready file: Address, Asset/Outlet type, Floor, Room, Unit | **Download Import-Ready** (Ready rows only) + **Download Needs Review** (Review Required rows only); Blocked excluded (`utils/importReadyExport.ts`) |
...
| Gaps or uncertainties | "Critical Errors" + "Uncertainties (Warnings)" KPIs and per-row Issues badges |
...
- **Automation proposes, the human approves.** Column mapping is auto-detected but shown for confirmation; AI mapping suggestions come with a confidence score and rationale, and are never applied without an explicit click. Manual edits can be discarded with **Reset to Suggested**.
- **Uncertainty degrades gracefully.** Confidently parsed rows are Ready; a single missing location field means Review Required (a human looks); unknown asset types or badly incomplete locations mean Blocked (kept out of the import-ready file entirely).
...
- **Reset to Suggested** restores the automatic keyword-based mapping after manual changes, so the user can experiment without losing the original recommendations.
...
## Import-ready & needs-review files

Two CSV downloads share the same columns — exactly the fields the case brief lists:

| Address | Asset Type | Floor | Room | Unit |
|---|---|---|---|---|

| Download | Who's in it | Purpose |
|---|---|---|
| **Import-Ready** | Only **Ready** rows | Clean file the platform can ingest |
| **Needs Review** | Only **Review Required** rows | Human queue for uncertain-but-complete-enough records |
| *(neither)* | **Blocked** rows | Kept in the app only — missing/unclassifiable data shouldn't ship downstream |
...
- Implemented in `utils/importReadyExport.ts` / `components/ExportDownloadButtons.tsx`.
...
- **Reset to Suggested**: restores the automatic column mapping after manual edits
...
- **Download Import-Ready / Needs Review**: two CSVs with the platform fields (Address, Asset Type, Floor, Room, Unit) — Ready-only for ingest, Review Required-only for the human queue; Blocked never exported
...
- **Import Ready Preview**: Ready / Review Required / Blocked tabs, each a detailed table (Sheet, Row #, Address, Building No, Unit, Floor, Room, Asset Type, Quantity, Original Raw Text, Issues) for tracing records back to the source file
...
components/
  ...
  ColumnMappingStep.tsx         # "Column Mapping" — auto-detected, per-column overrides, reset + AI suggestions
  ...
  ExportDownloadButtons.tsx     # "Download Import-Ready" + "Download Needs Review" CSVs
...
utils/
  ...
  importReadyExport.ts          # Builds Ready-only and Review-only CSV exports
